...
public class Controller {
    ...
    public Controller() throws IOException {
        dialog = new Dialog();      //登录窗口
        dialogExec();               //登录界面的运行-本类方法
        dialog.show();              //显示窗口
        System.out.println("显示");
        register = new Register();  //账号注册界面
        headPorTrait = new HeadPorTrait();//更改头像类
        forget = new Forget();      //忘记密码
        database = new Database();  //连接和调用数据库
        userdata = new userData();  //数据封装调用
        homepage = new homePage();  //个人主页界面
        addfriend = new addFriend();//添加好友
        friendpage = new friendPage();//好友的主页界面
        mainwindow = new mainWindow();//聊天窗口
        ch = new chat();
        CAGE =new cacuAge();
        alert = new Alert();
        //定义消息集合
        messageData.msg = new Vector<>();//消息
        messageData.msgTip = new HashMap<>();//消息提示
        messageData.account = new Vector<>();//消息数量
        database.connect();
        //如果有记住密码的就把他读到文本框上
        try{
            ResultSet ret = database.execResult("select * from save_pass");//调用自方法，返回所有数据结果集
            if(ret.next()){
                ResultSet ret1 = database.execResult("select * from dialog");
                while(ret1.next()){
                    if(ret.getString("account").equals(ret1.getString("account"))){//若save_pass和dialog的account一致
                        if (!ret.next()){//如果不为空返回
                            return;
                        }
                    }
                ...
                setHeadPortrait(((Button) $(dialog,"head")),ret.getString("head"));//头像
                ((Button) $(dialog,"save_pass1")).setVisible(false);//这里有两个属性，visible表示是否可见，并不会真正的隐藏，
                ((Button) $(dialog,"save_pass1")).setManaged(false);//而managed这个属性则表示是否隐藏，这两个属性必须同时使用
    ...
    public void exec()throws IOException{
        ...
        initEvent();        //整个程序事件的初始化
        dialogExec();       //登录界面的运行
        registerExec();     //注册界面的运行
        getForgetCode();    //获取忘记密码验证码
        getRegisterCode();  //获取注册验证码
        searchFriend();     //查找好友
        addFriends();       //查找并添加好友到列表
        forgetExec();       //忘记密码界面的运行
        sendMsg();          //发送消息的方法
        alterPersonPage();  //修改个人信息的方法
        chatManager.getInstance().setMainwindow(mainwindow);//向服务端发送数据并设置主界面
        changeHead();       //更换头像的方法
    }
        ...
        ((Button) $(register,"code")).setOnAction(event -> {
            ...
            try{//中国手机号正则
                ...
                if (!phone.equals("") && Pattern.matches(phoneRegExp,phone)){
                    codeRegister = new GetMessageCode().getCode(phone);//获取验证码
                ...
                try {
                    ...
                    if(!resultSet.next()){
                        if(password.equals(rePassword)){
                            ...
                            database.exec("insert into user values(?,?,?,?,?,?,?,?,?,?,?)",account,nickname,name,password,birthday,sex,userdata.getHead(),"广东省深圳市","",phone,"background6");
                    ...
                    if(resultSet.next()){
                        if(resultSet.getString("password").equals(password)){
                            ...
                            if(set.next()){
                                ...
                                //设置聊天助手
                                try {
                                    mainwindow.addFriend("助手小Q","head9","助手小Q","快来找我聊天吧!");
                                    ((Button) $(Controller.ch,"friendMore")).setText("助手小Q");
                                }catch (IOException e){
                                    e.printStackTrace();
                                }
                                messageData.msg.add(new Vector<>());
                                messageData.account.add("助手小Q");
                                messageData.msgTip.put("助手小Q",0);
                                //把所有好友添加到列表
                                ...
                                /*offlinemsg 离线信息 companion好友信息  user好友列表*/
                                    ...
                                    while (resultSet6.next()){
                                        if(resultSet6.getString("Y_account").equals(resultSet4.getString("I_account"))){
                                            ...
                                            if(resultSet5.next()){
                                                ...
                                                /*msg消息集合 msgMap好友消息映射 msgTip消息提示和数量*/
    ...
    public void sendMsg(){
        ((Button) $(ch,"send")).setOnAction(event -> {
            ...
            try{
                ...
                if(resultSet.next() && !remark.equals("助手小Q")){
            ...
            if(!text.equals("")){
                try{
                    if (Y_account.equals("助手小Q")){
                        try {
                            String reply = (new TuLing().robot(text)).toString();
                            ch.addRight(userdata.getHead(),text); //我的消息
                            ch.addLeft("head9",reply); //助手的自动回复消息
                            messageData.msg.get(0).add(userdata.getAccount() + " " + text);
                            messageData.msg.get(0).add(Y_account + " " + reply);
