...
public class friendList {
     ...
     * @throws IOException
     *///主界面--消息--（头像、账号、标记、信息）
    public friendList(String fHead,String fAccount,String remark,String label2) throws IOException {
        ...
        information = new Label();//好友列表上的备注 remark
        ...
        MsgTip = new Button();//消息提示+数量 或 无消息
    ...
    public void setMenuList(){
        ...
        for(int i=0;i<items.size();i++){
            menu.getItems().add(items.get(i));//添加对象数组内的菜单项对象
        }
    ...
    public void addMsgTip(int value){
        ...
        if (!MsgTip.getText().equals("")){
            value = value + Integer.parseInt(MsgTip.getText());
        }
    ...
    /**
     * 修改好友备注
     *///setActionForFriendPage和friendList
     ...
     * @param nickname 好友昵称
     */
    public void setActionForFriendPage(Database database,friendPage friendpage,String nickname,String Account){
        items.get(1).setOnAction(event -> {
                if(friendpage.isShowing()){//scence方法
                    friendpage.close();
                }
                try{//select逐条选择数据.next() 获取nickname昵称的所有信息
                    ResultSet resultSet = database.execResult("select * from user where nickname = ?",nickname);
                    if(resultSet.next()){//修改好友备注自方法
                        editFriendRemark(friendpage,database,resultSet.getString("account"),Account);
                        friendpage.setFriendPage(resultSet,information.getText());//设置主页的所有信息
                        friendpage.setNoAction();//内容不可编辑
                        friendpage.show();
                    }
                }catch (SQLException e){
                    e.printStackTrace();
    ...
    public void clearMsg(mainWindow mainwindow){
        items.get(2).setOnAction(event -> {
            int index = messageData.account.indexOf(friendName);//获取账号好友信息索引
            if(index!=-1){//索引不为空
                int index1 = mainwindow.getFriendlist().getSelectionModel().getSelectedIndex();//获取右键的好友索引
                messageData.msg.get(index).clear();//清除好友信息列表
                messageData.msgTip.remove(friendName);//移除提示标签
                messageData.msgMap.remove(friendName);//移除好友消息
                if(index == index1){
    ...
    public void removeFriend(Database database,mainWindow mainwindow,String account){
        items.get(3).setOnAction(event -> {
            int i = messageData.account.indexOf(friendName);//获取账号好友信息索引
            if(i != -1){
                if(i == 0){
                    alert.setText("这是助手，不能删除！");//默认一个好友助手
                    alert.exec();
                }else {
