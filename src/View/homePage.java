...
public class homePage extends Window{
    ...
    public homePage() throws IOException{
        ...
        move();//重写父类额外添加右键菜单功能
        setIcon();
        ((Button) $("alter")).setTooltip(new Tooltip("编辑资料"));//悬浮提示提示(笔写图片)
        alterAction();//资料可编辑
        close1();
        mini();
        mainwindow = new mainWindow();//聊天界面
        rightItem = new Vector<>();
        contextMenu = new ContextMenu();//右键菜单
    ...
    public void move(){
        root.setOnMousePressed(event -> {
            ...
            if(event.getButton() == MouseButton.SECONDARY){//添加右键事件
                rightItem.get(0).setOnAction(event1 -> {//添加右键菜单对象1
                    String Head = "head" + rand();
                    setHeadPortrait(((Button) $("head")),Head);//自方法，通过CSS设置随机头像
                    mainwindow.setHead(Head);//聊天窗口设置头像，同步
                    try{//引用外部类静态对象，更新个人用户信息(头像，账号) 通配符？匹配头像和用户
                        Controller.database.exec("update user set head = ? where account = ?",Head,((TextField) $("account")).getText());
                    }catch (SQLException e){
                        e.printStackTrace();
                    }
                });
                rightItem.get(1).setOnAction(event1 -> {//菜单2 修改背景
                    String Background = "background" + rand();
                    setBackground(((Pane) $("background")),"background" + rand());//Pane面板非按钮
                    try{
                ...
                contextMenu.show(this,event.getScreenX(),event.getScreenY());//显示在鼠标坐标位置
            }
    ...
    public void alterAction(){
        ((Button) $("alter")).setOnAction(event -> {
            ...
            ((Button) $("submit")).setVisible(true);//“完成”
